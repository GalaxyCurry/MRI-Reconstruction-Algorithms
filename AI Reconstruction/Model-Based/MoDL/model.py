...
class Aclass:
    ...
    def myAtA(self, img):
        ...
        return coilComb  # 采样kspace,重建,合并成一张图像域数据[1,1,256,232,1] +  lam*原始图像域数据[1,1,256,232,1]
...
def myCG(A, rhs):
    ...
    """
    rhs = r2c(rhs)  # 双实值图像域数据 --》单复值图像域数据
    cond = lambda i, rTr, *_: tf.logical_and(tf.less(i, 10), rTr > 1e-10)

    def body(i, rTr, x, r, p):
        with tf.name_scope('cgBody'):
            Ap = A.myAtA(p)
            alpha = rTr / tf.to_float(tf.reduce_sum(tf.conj(p) * Ap))   # 计算张量r的模平方和(L2 norm)//张量Ap的模平方和(L2 norm)
            alpha = tf.complex(alpha, 0.)
            x = x + alpha * p
            r = r - alpha * Ap  # 图像域数据 - 卷绕图像域数据
            rTrNew = tf.to_float(tf.reduce_sum(tf.conj(r) * r))  # 更新张量r的模平方和(L2 norm)
            beta = rTrNew / rTr
    ...
    x = tf.zeros_like(rhs)
    i, r, p = 0, rhs, rhs
    rTr = tf.to_float(tf.reduce_sum(tf.conj(r) * r), )  # 计算张量r的模平方和(L2 norm)
    loopVar = i, rTr, x, r, p
    out = tf.while_loop(cond, body, loopVar, name='CGwhile', parallel_iterations=1)[2]   # 返回x
    return c2r(out)   # 单复值图像域数据 --》双实值图像域数据
...
def makeModel(atb, csm, mask, training, nLayers, K, gradientMethod):
    ...
    with tf.name_scope('myModel'):
        with tf.variable_scope('Wts', reuse=tf.AUTO_REUSE):  # 定义名为'Wts'的变量作用域,实现变量的自动重用
            for i in range(1, K + 1):
                j = str(i)
                out['dw' + j] = dw(out['dc' + str(i - 1)], training, nLayers)  # 经过特征学习的双通道图像域数据
                lam1 = getLambda()
                rhs = atb + lam1 * out['dw' + j]  # 原始双通道图像域数据 + 经过特征学习的双通道图像域数据
